...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csvFile'])) {
    ...
    // Count lines in CSV to determine processing method
    $lineCount = 0;
    $tempHandle = fopen($tempFile, 'r');
    if ($tempHandle) {
        while (!feof($tempHandle)) {
            if (fgets($tempHandle) !== false) {
                $lineCount++;
            }
        }
        fclose($tempHandle);
    ...
    if ($lineCount > 1000) {
        echo '<div class="alert alert-warning">Large file detected (' . $lineCount . ' lines). Using parallel processing for optimal performance...</div>';
        flush();
        $processingResult = processCSVParallel($tempFile, $outputFile, $geminiApiKey, $grokApiKey, $openaiApiKey);
    } else {
...
function processCSVParallel($inputFile, $outputFile, $geminiApiKey, $grokApiKey, $openaiApiKey) {
    $input = fopen($inputFile, 'r');
    $output = fopen($outputFile, 'w');
    ...
    $headers = fgetcsv($input);
    if (!$headers) {
        echo '<div class="alert alert-danger">Invalid CSV format - no headers found</div>';
        fclose($input);
        fclose($output);
        return false;
    }
    ...
    $totalWorkOrders = count($workOrders);
    echo '<div class="alert alert-info">Processing ' . $totalWorkOrders . ' work orders with ' . $totalLines . ' total lines...</div>';
    flush();

    // Determine if parallel processing is needed
    if ($totalLines > 1000) {
        echo '<div class="alert alert-warning">Large file detected (' . $totalLines . ' lines). Using parallel processing with rate limiting...</div>';
        flush();

        // Process in parallel batches
        $batchSize = 5; // Max 5 concurrent requests
        $rateLimitDelay = 200000; // 200ms between batches = 5 requests per second max
        $workOrderBatches = array_chunk($workOrders, $batchSize, true);

        foreach ($workOrderBatches as $batchIndex => $batch) {
            $batchStartTime = microtime(true);
            echo '<div class="progress-info">Processing batch ' . ($batchIndex + 1) . ' of ' . count($workOrderBatches) . '...</div>';
            flush();

            // Process batch in parallel using multi-curl
            $results = processWorkOrderBatchParallel($batch, $columnMap, $geminiApiKey, $grokApiKey, $openaiApiKey);

            // Write results to file
            foreach ($results as $workOrderNum => $result) {
                if (isset($workOrders[$workOrderNum])) {
                    writeWorkOrderResults($output, $workOrders[$workOrderNum], $result, $headers);
                    $processedWorkOrders[] = $workOrderNum;
            ...
            $batchElapsed = (microtime(true) - $batchStartTime) * 1000000; // in microseconds
            if ($batchElapsed < $rateLimitDelay) {
                usleep($rateLimitDelay - $batchElapsed);
            }
        }
    } else {
        // Use regular sequential processing for smaller files
        echo '<div class="alert alert-info">Using standard sequential processing...</div>';
        flush();
        ...
        foreach ($workOrders as $workOrderNum => $rows) {
            ...
            try {
                $analysis = analyzeWorkOrder($rows, $columnMap, $geminiApiKey, $grokApiKey, $openaiApiKey);
                writeWorkOrderResults($output, $rows, $analysis, $headers);
    ...
    echo '<div class="alert alert-success">Processing complete!</div>';
    return true;
}

function processWorkOrderBatchParallel($batch, $columnMap, $geminiApiKey, $grokApiKey, $openaiApiKey) {
    $multiHandle = curl_multi_init();
    $curlHandles = [];
    $results = [];

    // Create curl handles for each work order in batch
    foreach ($batch as $workOrderNum => $rows) {
        $prompt = buildWorkOrderPrompt($rows, $columnMap);
    ...
    foreach ($curlHandles as $workOrderNum => $ch) {
        ...
        if ($httpCode == 200 && $response) {
            ...
            if (isset($data['choices'][0]['message']['content'])) {
                $parsedResult = parseDetailedAIResponse($data['choices'][0]['message']['content'], $batch[$workOrderNum], $columnMap);
                $results[$workOrderNum] = $parsedResult;
            } else {
                // Create error result
                $results[$workOrderNum] = createErrorResult($batch[$workOrderNum], $columnMap);
            }
        } else {
            // Create error result
            $results[$workOrderNum] = createErrorResult($batch[$workOrderNum], $columnMap);
...
function buildWorkOrderPrompt($rows, $columnMap) {
    // Build the prompt for the work order
    $prompt = getDetailedAnalysisPrompt();
    $prompt .= "\n\nWork Order Details:\n";

    foreach ($rows as $row) {
        $description = isset($columnMap['description']) ? $row[$columnMap['description']] : 'N/A';
        $unitCost = isset($columnMap['unit_cost']) ? $row[$columnMap['unit_cost']] : 0;
        $quantity = isset($columnMap['quantity']) ? $row[$columnMap['quantity']] : 1;
        $lineCost = isset($columnMap['line_cost']) ? $row[$columnMap['line_cost']] : 0;

        $prompt .= "- Item: $description\n";
        $prompt .= "  Unit Cost: $unitCost, Quantity: $quantity, Line Cost: $lineCost\n";
    }
...
function mapColumns($headers) {
    ...
    foreach ($headers as $index => $header) {
        ...
        if (strpos($headerLower, 'work') !== false && strpos($headerLower, 'order') !== false) {
            $columnMap['work_order'] = $index;
        }

        if (strpos($headerLower, 'ifm') !== false && strpos($headerLower, 'invoice') !== false && strpos($headerLower, 'description') !== false) {
            $columnMap['description'] = $index;
        }
...
function writeWorkOrderResults($output, $rows, $analysis, $headers) {
    // Write results for each row in the work order
    foreach ($rows as $index => $row) {
        $lineAnalysis = isset($analysis['line_items'][$index]) ? $analysis['line_items'][$index] : [];

        $extendedRow = array_merge($row, [
            isset($lineAnalysis['capex_percent']) ? $lineAnalysis['capex_percent'] : '0',
            isset($lineAnalysis['opex_percent']) ? $lineAnalysis['opex_percent'] : '100',
            isset($lineAnalysis['capex_amount']) ? $lineAnalysis['capex_amount'] : '0',
            isset($lineAnalysis['opex_amount']) ? $lineAnalysis['opex_amount'] : '0',
            isset($lineAnalysis['justification']) ? $lineAnalysis['justification'] : '',
        ...
        while (count($extendedRow) < count($headers) + 10) {
            $extendedRow[] = '';
        }

        fputcsv($output, $extendedRow);
    }
}

function createErrorResult($rows, $columnMap) {
    ...
    return [
        ...
        'total_capex' => 0,
        'total_opex' => $subtotal,
        'justification' => 'Error processing work order',
        'category' => 'N/A',
        'line_items' => []
    ];
}

function getDetailedAnalysisPrompt() {
